# Need to apply some changes to an internal derivation, so just copy the entire build file for hip from nixpkgs
# https://github.com/NixOS/nixpkgs/blob/22.05/pkgs/development/compilers/hip/default.nix
{ stdenv
, binutils-unwrapped
, clang
, clang-unwrapped
...
let
  hip = stdenv.mkDerivation rec {
    ...
    prePatch = ''
      ...
      sed -e 's,''${_IMPORT_PREFIX}/../llvm/lib/clang/\*/include,${clang-unwrapped}/lib/clang/*/include,' \
...
stdenv.mkDerivation rec {
  ...
  prePatch = ''
    sed -e 's,''${HIP_CLANG_ROOT}/lib/clang/\*/include,${clang-unwrapped}/lib/clang/*/include,' \
        -e '/hip_add_interface_compile_flags(hip::device -x hip)/ahip_add_interface_compile_flags(hip::device --hip-device-lib-path=\\"''${AMD_DEVICE_LIBS_PREFIX}\/amdgcn\/bitcode\\\")' \
        -e 's,''${HIP_CLANG_ROOT}/lib/clang/\*/lib/\*,${compiler-rt}/lib/,' \
